	...
	public void addCheckedInformation(final ItemStack is, final World world, final List<String> list, final ITooltipFlag advancedTooltips) {
		...
		if (WTApi.instance().getConfig().isInfinityBoosterCardEnabled()) {
			if (WTApi.instance().getConfig().isOldInfinityMechanicEnabled()) {
				...
				final String infinityEnergyString = WFTUtils.isWFTCreative(is) ? I18n.format(WTApi.instance().getConstants().getTooltips().infinite()) : isShiftKeyDown() ? "" + infinityEnergyAmount + "" + TextFormatting.GRAY + " " + I18n.format(WTApi.instance().getConstants().getTooltips().units()) : ReadableNumberConverter.INSTANCE.toSlimReadableForm(infinityEnergyAmount);
